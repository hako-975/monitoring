...
function updateRealTimeTable(data) {
    ...
    headerRow.append($('<th>').text('LT (Left Top)'));
    headerRow.append($('<th>').text('RT (Right Top)'));
    headerRow.append($('<th>').text('LD (Left Down)'));
    headerRow.append($('<th>').text('RD (Right Down)'));
    headerRow.append($('<th>').text('Create At'));

    // Loop through the data and populate the table rows
    for (var i = 0; i < data.label_servo_array.length; i++) {
        var row = $('<tr>');
        row.append($('<td>').text(i + 1 + ".")); // No.
        row.append($('<td>').text(data.lt_array[i])); // Arus
        row.append($('<td>').text(data.rt_array[i])); // Tegangan
        row.append($('<td>').text(data.ld_array[i])); // Tegangan
        row.append($('<td>').text(data.rd_array[i])); // Tegangan
        row.append($('<td>').text(data.label_servo_array[i])); // Create At
...
function fetchRealTimeData() {
    $.ajax({
        url: `fetch_data_array_servo_no_limit.php?filter=${currentFilter}`, // Path to server-side script with filter parameter
        type: 'GET',
        dataType: 'json',
